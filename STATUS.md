...
- [x] Module 10「マルチエージェント」拡充（Phase B1で統合済み）
  ...
  - 確認クイズ5問追加（計10問）
- [x] Module 11「コンテキスト管理と信頼性」拡充（4セクション追加+クイズ5問追加）
  - Rate Limits: Token Bucket・Service Tiers・429対策・指数バックオフ
  - Guardrails: 入出力フィルタリング・コンテンツ分類・多層防御
  - プライバシー: PII検出・データ最小化・監査ログ・コンプライアンス
  - 高度コンテキスト管理: Contextual Retrieval・プロンプトキャッシング・Citations
  - 用語集追加、確認クイズ5問追加（計10問）
- [x] 問題集160問追加（D1:27問+D2:18問+D3:20問+D4:20問+D5:15問+シナリオ:60問）
  - ドメイン別問題集100問: D1（27問）・D2（18問）・D3（20問）・D4（20問）・D5（15問）
  - シナリオベース問題集60問: 6シナリオ×10問（実務場面想定・ストーリー展開型）
    - S1: 社内チャットボット導入（D1+D4）
    - S2: コード自動レビューシステム（D3+D1）
    - S3: カスタマーサポート自動化（D2+D5）
    - S4: マルチエージェント文書処理（D1+D2）
    - S5: API統合データパイプライン（D2+D5）
    - S6: エンタープライズAIガバナンス（D5+D3）

## 次にやること

- M11拡充+160問追加完了。残りのモジュール拡充（M1〜M4, M7, M8, M12等）へ
- 問題集のindex.html統合（UIへの組み込み）
